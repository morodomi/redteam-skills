#!/bin/bash
#
# Test: xss-attacker structure
#
# TC-01: xss-attacker.md exists
# TC-02: YAML frontmatter (name, description, allowed-tools) exists
# TC-03: Detection targets section (Reflected XSS, Sanitization) exists
...
echo "================================"
echo "xss-attacker Structure Test"
echo "================================"
echo ""

# TC-01: xss-attacker.md exists
if [ -f "$AGENT_FILE" ]; then
    test_case "TC-01" "xss-attacker.md exists" 0
else
    test_case "TC-01" "xss-attacker.md exists" 1
fi
...
# TC-03: Detection targets section (Reflected XSS, Sanitization) exists
if [ -f "$AGENT_FILE" ]; then
    has_reflected=$(grep -ci "reflected" "$AGENT_FILE" 2>/dev/null || echo 0)
    has_sanitiz=$(grep -ci "sanitiz" "$AGENT_FILE" 2>/dev/null || echo 0)

    if [ "$has_reflected" -gt 0 ] && [ "$has_sanitiz" -gt 0 ]; then
        test_case "TC-03" "Detection targets section (Reflected XSS, Sanitization) exists" 0
    else
        test_case "TC-03" "Detection targets section (Reflected XSS, Sanitization) exists" 1
    fi
else
    test_case "TC-03" "Detection targets section (Reflected XSS, Sanitization) exists" 1
fi
...
if [ -f "$AGENT_FILE" ]; then
    ...
    has_xss_patterns=$(grep -ci "echo\|innerHTML\|safe" "$AGENT_FILE" 2>/dev/null || echo 0)

    if [ "$has_patterns" -gt 0 ] && [ "$has_xss_patterns" -gt 0 ]; then
...
if [ -f "$AGENT_FILE" ]; then
    ...
    has_xss_type=$(grep -ci '"type".*xss\|"type".*reflected' "$AGENT_FILE" 2>/dev/null || echo 0)
...
# TC-08: CWE/OWASP mapping exists
if [ -f "$AGENT_FILE" ]; then
    has_cwe=$(grep -ci "CWE-79\|CWE" "$AGENT_FILE" 2>/dev/null || echo 0)
