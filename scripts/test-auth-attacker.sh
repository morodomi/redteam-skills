#!/bin/bash
#
# Test: auth-attacker structure
#
# TC-01: auth-attacker.md exists
# TC-02: YAML frontmatter (name, description, allowed-tools) exists
# TC-03: Detection targets section (Hardcoded Credentials, Missing Auth, Broken Access Control) exists
...
AGENT_FILE="$PROJECT_ROOT/plugins/redteam-core/agents/auth-attacker.md"
...
echo "================================"
echo "auth-attacker Structure Test"
echo "================================"
echo ""

# TC-01: auth-attacker.md exists
if [ -f "$AGENT_FILE" ]; then
    test_case "TC-01" "auth-attacker.md exists" 0
else
    test_case "TC-01" "auth-attacker.md exists" 1
fi
...
# TC-03: Detection targets section (Hardcoded Credentials, Missing Auth, Broken Access Control) exists
if [ -f "$AGENT_FILE" ]; then
    has_hardcoded=$(grep -ci "hardcoded" "$AGENT_FILE" 2>/dev/null || echo 0)
    has_missing_auth=$(grep -ci "missing.*auth\|auth.*missing" "$AGENT_FILE" 2>/dev/null || echo 0)
    has_access_control=$(grep -ci "access.*control" "$AGENT_FILE" 2>/dev/null || echo 0)

    if [ "$has_hardcoded" -gt 0 ] && [ "$has_missing_auth" -gt 0 ] && [ "$has_access_control" -gt 0 ]; then
        test_case "TC-03" "Detection targets section (Hardcoded Credentials, Missing Auth, Broken Access Control) exists" 0
    else
        test_case "TC-03" "Detection targets section (Hardcoded Credentials, Missing Auth, Broken Access Control) exists" 1
    fi
else
    test_case "TC-03" "Detection targets section (Hardcoded Credentials, Missing Auth, Broken Access Control) exists" 1
fi
...
if [ -f "$AGENT_FILE" ]; then
    ...
    has_auth_patterns=$(grep -ci "password\|credential\|secret\|api_key" "$AGENT_FILE" 2>/dev/null || echo 0)

    if [ "$has_patterns" -gt 0 ] && [ "$has_auth_patterns" -gt 0 ]; then
...
# TC-08: CWE/OWASP mapping exists
if [ -f "$AGENT_FILE" ]; then
    has_cwe=$(grep -ci "CWE-287\|CWE-862\|CWE" "$AGENT_FILE" 2>/dev/null || echo 0)
    has_owasp=$(grep -ci "OWASP\|A01:2021\|A07:2021" "$AGENT_FILE" 2>/dev/null || echo 0)
